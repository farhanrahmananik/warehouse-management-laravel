...
@section('title', 'Create Stock In - ' . config('app.name'))
...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between gap-3 mb-4">
        <div>
            <h1 class="h3 mb-1">Create Stock In</h1>
            <p class="text-muted mb-0">Record received stock into a warehouse.</p>
        </div>

        <a href="{{ route('stock-ins.index') }}" class="btn btn-outline-secondary">Back</a>
    </div>

    @if ($warehouses->isEmpty())
        <div class="alert alert-warning" role="alert">
            Please create an active warehouse before creating a stock in document.
        </div>
    @endif

    @if ($products->isEmpty())
        <div class="alert alert-warning" role="alert">
            Please create an active product before creating a stock in document.
        </div>
    @endif

    <form method="POST" action="{{ route('stock-ins.store') }}" class="card shadow-sm border-0">
        @csrf
        ...
        <div class="card-body">
            ...
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-2 mb-3">
                <div>
                    ...
                    <p class="small text-muted mb-0">Add each product once with a positive received quantity.</p>
                </div>
                <button type="button" class="btn btn-outline-primary btn-sm" id="add-stock-in-item">Add Item</button>
            </div>
            ...
            <div id="stock-in-items">
                @foreach ($items as $index => $item)
                    <div class="stock-in-item border rounded p-3 mb-3" data-item-row>
        ...
        <div class="card-footer bg-white d-flex justify-content-end gap-2">
            <a href="{{ route('stock-ins.index') }}" class="btn btn-outline-secondary">Cancel</a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const itemsContainer = document.getElementById('stock-in-items');
            const itemTemplate = document.getElementById('stock-in-item-template');
            const addItemButton = document.getElementById('add-stock-in-item');
            ...
            itemsContainer.addEventListener('click', function (event) {
                ...
                if (itemsContainer.querySelectorAll('[data-item-row]').length === 1) {
                    alert('At least one stock in item is required.');
                    return;
                }
